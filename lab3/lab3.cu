...
int thread2Dpx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if ( mask[thread1Dp] > 127.0f)
  {
  	...
  	output[thread1Dp*3+0]=output[thread1Dp*3+0]/(float)count;
  	output[thread1Dp*3+1]=output[thread1Dp*3+1]/(float)count;
  	output[thread1Dp*3+2]=output[thread1Dp*3+2]/(float)count;
  }
}
